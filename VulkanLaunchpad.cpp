...
std::string to_string(glslang_stage_t stage) {
    switch (stage) {
        ...
        case GLSLANG_STAGE_INTERSECT_NV:
            return "GLSLANG_STAGE_INTERSECT_NV";
        case GLSLANG_STAGE_ANYHIT_NV:
            return "GLSLANG_STAGE_ANYHIT_NV";
        case GLSLANG_STAGE_CLOSESTHIT_NV:
            return "GLSLANG_STAGE_CLOSESTHIT_NV";
        case GLSLANG_STAGE_MISS_NV:
            return "GLSLANG_STAGE_MISS_NV";
        case GLSLANG_STAGE_CALLABLE_NV:
            return "GLSLANG_STAGE_CALLABLE_NV";
        case GLSLANG_STAGE_TASK_NV:
            return "GLSLANG_STAGE_TASK_NV";
        case GLSLANG_STAGE_MESH_NV:
            return "GLSLANG_STAGE_MESH_NV";
        default:
...
std::tuple<vk::ShaderModule, vk::PipelineShaderStageCreateInfo>
loadShaderFromSpirvAndCreateShaderModuleAndStageInfo(const uint32_t *spirv, size_t byteSize,
...
std::tuple<vk::ShaderModule, vk::PipelineShaderStageCreateInfo>
loadShaderFromMemoryAndCreateShaderModuleAndStageInfo(const std::string &shaderCode, const std::string &shaderName,
                                                      const vk::ShaderStageFlagBits shaderStage) {
#ifdef USE_SHADERC
    ...
    return loadShaderFromSpirvAndCreateShaderModuleAndStageInfo(spirv.data(),
                                                                spirv.size() * sizeof(decltype(spirv)::value_type),
                                                                shaderStage);
}

std::tuple<vk::ShaderModule, vk::PipelineShaderStageCreateInfo>
loadShaderFromFileAndCreateShaderModuleAndStageInfo(const std::string &shader_filename,
                                                    const vk::ShaderStageFlagBits shaderStage) {
    std::string path = {};
...
VkPipeline createGraphicsPipelineInternal(
    ...
    auto vertTpl = loadShadersFromMemoryInstead
                   ? loadShaderFromMemoryAndCreateShaderModuleAndStageInfo(config.vertexShaderPath,
                                                                           "vertex shader from memory",
                                                                           vk::ShaderStageFlagBits::eVertex)
                   : loadShaderFromFileAndCreateShaderModuleAndStageInfo(config.vertexShaderPath,
                                                                         vk::ShaderStageFlagBits::eVertex);
    ...
    auto fragTpl = loadShadersFromMemoryInstead
                   ? loadShaderFromMemoryAndCreateShaderModuleAndStageInfo(config.fragmentShaderPath,
                                                                           "fragment shader from memory",
                                                                           vk::ShaderStageFlagBits::eFragment)
                   : loadShaderFromFileAndCreateShaderModuleAndStageInfo(config.fragmentShaderPath,
                                                                         vk::ShaderStageFlagBits::eFragment);
    ...
    std::array<vk::PipelineShaderStageCreateInfo, 2> shaderStages{std::get<vk::PipelineShaderStageCreateInfo>(vertTpl),
                                                                  std::get<vk::PipelineShaderStageCreateInfo>(fragTpl)};
    // Describe the vertex input, i.e. two vertex input attributes in our case:

    std::vector<vk::VertexInputBindingDescription> inputBufferBindings(std::begin(config.vertexInputBuffers),
                                                                       std::end(config.vertexInputBuffers));
    std::vector<vk::VertexInputAttributeDescription> inputAttributeDescriptions(
            std::begin(config.inputAttributeDescriptions), std::end(config.inputAttributeDescriptions));

    auto vertexInputState = vk::PipelineVertexInputStateCreateInfo{}
            .setVertexBindingDescriptionCount(
                    static_cast<uint32_t>(inputBufferBindings.size())).setPVertexBindingDescriptions(
                    inputBufferBindings.data())
            .setVertexAttributeDescriptionCount(
                    static_cast<uint32_t>(inputAttributeDescriptions.size())).setPVertexAttributeDescriptions(
    ...
    auto viewport = vk::Viewport{}
            .setX(0.0f).setY(0.0f)
            .setWidth(static_cast<float>(mSwapchainConfig.imageExtent.width)).setHeight(
                    static_cast<float>(mSwapchainConfig.imageExtent.height))
    ...
    auto multisampleState = vk::PipelineMultisampleStateCreateInfo{}.setRasterizationSamples(
    ...
    auto colorBlendAttachmentState = vk::PipelineColorBlendAttachmentState{}
            ...
            .setColorWriteMask(
                    ...
                    vk::ColorComponentFlagBits::eA); // write all color components
    ...
    auto colorBlendState = vk::PipelineColorBlendStateCreateInfo{}.setAttachmentCount(1u).setPAttachments(
            &colorBlendAttachmentState);

    // But again: not so fast! We have to define the LAYOUT of our descriptors first
    std::vector<vk::DescriptorSetLayoutBinding> layoutBindings(std::begin(config.descriptorLayout),
                                                               std::end(config.descriptorLayout));
    auto descriptorSetLayout = mDevice.createDescriptorSetLayoutUnique(
            vk::DescriptorSetLayoutCreateInfo{}
                    ...
                    .setPBindings(layoutBindings.data()), nullptr, mDispatchLoader
    );

    std::vector<vk::PushConstantRange> pushConstantRanges(std::begin(config.pushConstantRanges),
                                                          std::end(config.pushConstantRanges));
    ...
    auto pipelineLayout = mDevice.createPipelineLayoutUnique(
            vk::PipelineLayoutCreateInfo{} // A pipeline's layout describes all resources used by a pipeline or in shaders.
                    ...
                    .setPPushConstantRanges(
                            pushConstantRanges.data())// We don't need the actual descriptors when defining the PIPELINE. The LAYOUT is sufficient at this point.
    ...
    auto pipelineCreateInfo = vk::GraphicsPipelineCreateInfo{}
            ...
            .setPDynamicState(
                    &dynamicStateCreateInfo); // <--- Which subpass of the given renderpass we are going to use this graphics pipeline for
    ...
    mPipelineLayouts[graphicsPipelineHandle] = std::forward_as_tuple(std::move(descriptorSetLayout),
...
VkPipeline vklCreateGraphicsPipeline(
    ...
    auto graphicsPipelineHandle = createGraphicsPipelineInternal(config, loadShadersFromMemoryInstead,
                                                                 static_cast<vk::PrimitiveTopology>(topology));
    if (VK_NULL_HANDLE == graphicsPipelineHandle) {
        VKL_EXIT_WITH_ERROR(
                "Failed to create graphics pipeline. Check console output if there were any problems with shader compilation!");
    }
    ...
    mUserKnownPipelines[graphicsPipelineHandle] = std::make_tuple(config, std::string(config.vertexShaderPath),
                                                                  std::string(config.fragmentShaderPath),
                                                                  loadShadersFromMemoryInstead);
    return graphicsPipelineHandle;
}
    ...
    auto memoryAllocInfo = vk::MemoryAllocateInfo{}
            ...
            .setMemoryTypeIndex([&]() {
                ...
                if (-1 == selectedMemIndex) {
                    VKL_EXIT_WITH_ERROR(
                            std::string("ERROR: Couldn't find suitable memory of size[") + std::to_string(bufferSize) +
                            "] and requirements[" + std::to_string(memoryRequirements.alignment) + ", " +
                            std::to_string(memoryRequirements.memoryTypeBits) + ", " +
...
VkMemoryAllocateInfo vklCreateMemoryAllocateInfo(VkDeviceSize bufferSize, VkMemoryRequirements memoryRequirements,
    ...
    return static_cast<VkMemoryAllocateInfo>(vklCreateMemoryAllocateInfo(static_cast<vk::DeviceSize>(bufferSize),
                                                                         static_cast<vk::MemoryRequirements>(memoryRequirements),
                                                                         static_cast<vk::MemoryPropertyFlags>(memoryPropertyFlags)));
}
...
VkDeviceMemory vklAllocateMemoryForGivenRequirements(VkDeviceSize bufferSize, VkMemoryRequirements memoryRequirements,
    ...
    if (returnCode == VK_SUCCESS) {
        ...
        VKL_EXIT_WITH_ERROR(
                std::string("Error allocating memory of size [") + std::to_string(bufferSize) + "] and requirements[" +
                std::to_string(memoryRequirements.alignment) + ", " +
                std::to_string(memoryRequirements.memoryTypeBits) + ", " + std::to_string(memoryRequirements.size) +
                "]\n    Error Code: " + to_string(returnCode));
    }
}

vk::UniqueDeviceMemory
vklAllocateMemoryForGivenRequirements(vk::DeviceSize bufferSize, vk::MemoryRequirements memoryRequirements,
                                      vk::MemoryPropertyFlags memoryPropertyFlags) {
    const auto memoryAllocInfo = vklCreateMemoryAllocateInfo(bufferSize, memoryRequirements, memoryPropertyFlags);
...
VkBuffer vklCreateHostCoherentBufferWithBackingMemory(VkDeviceSize buffer_size, VkBufferUsageFlags buffer_usage) {
    ...
    auto memory = vklAllocateMemoryForGivenRequirements(static_cast<vk::DeviceSize>(buffer_size),
...
void vklDestroyHostCoherentBufferAndItsBackingMemory(VkBuffer buffer) {
    ...
    if (mHostCoherentBuffersWithBackingMemory.end() != search) {
        ...
        VKL_WARNING(
                "VkDeviceMemory for the given VkBuffer not found. Are you sure that you have created this buffer with vklCreateHostCoherentBufferWithBackingMemory(...)? Are you sure that you haven't already destroyed this VkBuffer?");
    }
...
void vklDestroyDeviceLocalBufferAndItsBackingMemory(VkBuffer buffer) {
    ...
    if (mDeviceLocalBuffersWithBackingMemory.end() != search) {
        ...
        VKL_WARNING(
                "VkDeviceMemory for the given VkBuffer not found. Are you sure that you have created this buffer with vklCreateDeviceLocalBufferWithBackingMemory(...)? Are you sure that you haven't already destroyed this VkBuffer?");
    }
...
void vklCopyDataIntoHostCoherentBuffer(VkBuffer buffer, size_t buffer_offset_in_bytes, const void *data_pointer,
    ...
    if (mHostCoherentBuffersWithBackingMemory.end() == search) {
        VKL_EXIT_WITH_ERROR(
                "Couldn't find backing memory for the given VkBuffer => Can't copy data. Have you created the buffer via vklCreateHostCoherentBufferWithBackingMemory(...)?");
    }
...
#else
    uint8_t *mappedMemory = static_cast<uint8_t *>(mDevice.mapMemory(search->second.get(), 0,
                                                                     static_cast<vk::DeviceSize>(data_size_in_bytes)));
    mappedMemory += buffer_offset_in_bytes;
    memcpy(mappedMemory, data_pointer, data_size_in_bytes);
...
void vklNotifyResized(const VklSwapchainConfig &swapchain_config) {
    ...
    if (surface_capabilities.currentExtent.width != mSwapchainConfig.imageExtent.width ||
        surface_capabilities.currentExtent.height != mSwapchainConfig.imageExtent.height) {
        return;
    }
    mDevice.waitIdle();
...
void vklBindDescriptorSetToPipeline(VkDescriptorSet descriptor_set, VkPipeline pipeline) {
    ...
    if (mSingleUseCommandBuffers.empty()) {
        VKL_EXIT_WITH_ERROR(
                "There are no command buffers to record commands into. Have you called vklStartRecordingCommands() beforehand?");
    }
    ...
    if (mPipelineLayouts.end() == searchPl) {
        VKL_EXIT_WITH_ERROR(
                "Couldn't find the VkPipeline passed to vklBindDescriptorSetToPipeline. Is it a valid handle and has it been created with vklCreateGraphicsPipeline(...)?");
    }
...
void vklCreateRenderResources(const VklSwapchainConfig &swapchain_config) {
    ...
    if (swapchain_config.imageExtent.width != surfaceCapabilities.currentExtent.width ||
        swapchain_config.imageExtent.height != surfaceCapabilities.currentExtent.height) {
        std::cout << "WARNING: Swapchain config's extents[" << swapchain_config.imageExtent.width << "x"
                  << swapchain_config.imageExtent.height << "] do not match the surface capabilities' extents["
                  << surfaceCapabilities.currentExtent.width << "x" << surfaceCapabilities.currentExtent.height << "]"
                  << VKL_DESCRIBE_FILE_LOCATION_FOR_OUT_STREAM << "\n";
    }
    ...
    for (size_t i = 0; i < mSwapchainConfig.swapchainImages.size(); ++i) {
        ...
        if (mSwapchainConfig.swapchainImages[0].colorAttachmentImageDetails.imageHandle != VK_NULL_HANDLE) {
            attachments_0.push_back(mSwapchainConfig.swapchainImages[0].colorAttachmentImageDetails);
        }
        if (mSwapchainConfig.swapchainImages[0].depthAttachmentImageDetails.imageHandle != VK_NULL_HANDLE) {
            attachments_0.push_back(mSwapchainConfig.swapchainImages[0].depthAttachmentImageDetails);
        }

        auto hasColorAttachment =
                mSwapchainConfig.swapchainImages[i].colorAttachmentImageDetails.imageHandle != VK_NULL_HANDLE;
        auto hasDepthAttachment =
        ...
        if (hasColorAttachment) {
            attachments_i.push_back(mSwapchainConfig.swapchainImages[i].colorAttachmentImageDetails);
        }
        if (hasDepthAttachment) {
            attachments_i.push_back(mSwapchainConfig.swapchainImages[i].depthAttachmentImageDetails);
        }
        ...
        for (size_t j = 0; j < attachments_i.size(); ++j) {
            ...
            if (attachments_i[j].imageFormat != attachments_0[j].imageFormat) {
                VKL_EXIT_WITH_ERROR(
                        "Corresponding VklSwapchainImageDetails::imageFormat entries must be set to the same formats! However, element[" +
                        std::to_string(i) + ", " + std::to_string(j) + "] is set to "
                        + vk::to_string(static_cast<vk::Format>(attachments_i[j].imageFormat)) + ", while element[0, " +
                        std::to_string(j) + "] is set to " +
                        vk::to_string(static_cast<vk::Format>(attachments_0[j].imageFormat)));
            }
            if (attachments_i[j].imageUsage != attachments_0[j].imageUsage) {
                VKL_EXIT_WITH_ERROR(
                        "Corresponding VklSwapchainImageDetails::imageUsage entries must be set to the same values! However, element[" +
                        std::to_string(i) + ", " + std::to_string(j) + "] is set to "
                        + vk::to_string(static_cast<vk::ImageUsageFlags>(attachments_i[j].imageUsage)) +
                        ", while element[0, " + std::to_string(j) + "] is set to " +
                        vk::to_string(static_cast<vk::ImageUsageFlags>(attachments_0[j].imageUsage)));
            }
            ...
            if (0 == i) {
                ...
                attachmentDescriptions.emplace_back(vk::AttachmentDescription{}
                                                            .setFormat(
                                                                    static_cast<vk::Format>(attachments_i[j].imageFormat))
                                                            .setLoadOp(
                                                                    vk::AttachmentLoadOp::eClear)        // What do do with the image when the renderpass starts? => Make sure that we have cleared the content of previous frames!
                                                            .setStoreOp( // What to do with the image when the renderpass has finished? => We don't need the depth buffer for anything afterwards.
                                                                    (VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT &
                                                                     attachments_i[j].imageUsage) != 0
                                                                    ? vk::AttachmentStoreOp::eDontCare
                                                                    : vk::AttachmentStoreOp::eStore)
                                                            .setInitialLayout(
                                                                    vk::ImageLayout::eUndefined)    // When the renderpass starts, in which layout will the image be? => We don't care since we're clearing it.
                ...
                if ((VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT & attachments_i[j].imageUsage) != 0) {
                    depthAttachmentsInSubpass0.emplace_back(curAttachmentIndex,
                                                            vk::ImageLayout::eDepthStencilAttachmentOptimal); // Describes the index (w.r.t. attachmentDescriptions) and the desired layout of the depth attachment for subpass 0
                } else {
                    colorAttachmentsInSubpass0.emplace_back(curAttachmentIndex,
                                                            vk::ImageLayout::eColorAttachmentOptimal); // Describes the index (w.r.t. attachmentDescriptions) and the desired layout of the color attachment for subpass 0
                }
            }
    ...
    auto subpassDescription = vk::SubpassDescription{}
            .setPipelineBindPoint(
                    vk::PipelineBindPoint::eGraphics) // Despite looking as if it was configurable, only eGraphics is valid/supported
    ...
    if (mHasDepthAttachments) {
        ...
        mDstAccess0 |= (vk::AccessFlagBits::eDepthStencilAttachmentRead |
    ...
    std::array<vk::SubpassDependency, 2> subpassDependencies{
            vk::SubpassDependency{}
                    ...
                    .setSrcAccessMask(mSrcAccess0) /* -> */ .setDstAccessMask(mDstAccess0), vk::SubpassDependency{}
                    ...
                    .setSrcStageMask(vk::PipelineStageFlagBits::eColorAttachmentOutput) /* -> */ .setDstStageMask(
                    ...
                    .setSrcAccessMask(vk::AccessFlagBits::eColorAttachmentWrite) /* -> */ .setDstAccessMask(
                            vk::AccessFlags{})
    };
...
bool vklInitFramework(VkInstance vk_instance, VkSurfaceKHR vk_surface, VkPhysicalDevice vk_physical_device,
    ...
    if (swapchain_config.imageExtent.width < 128 || swapchain_config.imageExtent.height < 128) {
        VKL_EXIT_WITH_ERROR(
                "VkExtent2D passed to vklInitFramework through VklSwapchainConfig::imageExtent are too small (less than 128)");
    }
    if (swapchain_config.swapchainImages.empty()) {
        VKL_EXIT_WITH_ERROR(
                "No data about swapchain images passed to vklInitFramework through VklSwapchainConfig::swapchainImages");
    }
    for (int i = 0; i < swapchain_config.swapchainImages.size(); ++i) {
        if (swapchain_config.swapchainImages[i].colorAttachmentImageDetails.imageHandle == VK_NULL_HANDLE) {
            VKL_EXIT_WITH_ERROR(
                    "No/invalid color attachment image details passed to vklInitFramework through VklSwapchainConfig::swapchainImages[" +
                    std::to_string(i) + "]::colorAttachmentImageDetails");
        }
    }
    for (int i = 0; i < swapchain_config.swapchainImages.size(); ++i) {
        auto imageDetails = std::vector<VklSwapchainImageDetails>{
                swapchain_config.swapchainImages[i].colorAttachmentImageDetails,
                swapchain_config.swapchainImages[i].depthAttachmentImageDetails};
        for (int j = 0; j < imageDetails.size(); ++j) {
            ...
            if (VkFormat{} == imageDetails[j].imageFormat) {
                VKL_EXIT_WITH_ERROR(
                        "Invalid VkFormat passed to vklInitFramework through VklSwapchainConfig::swapchainImages[" +
                        std::to_string(i) + "]::imageDetails[" + std::to_string(j) + "]::imageFormat");
            }
            if (VkImageUsageFlags{} == imageDetails[j].imageUsage) {
                VKL_EXIT_WITH_ERROR(
                        "Invalid VkImageUsageFlags passed to vklInitFramework through VklSwapchainConfig::swapchainImages[" +
                        std::to_string(i) + "]::imageDetails[" + std::to_string(j) + "]::imageUsage");
            }
        }
    }
    ...
    for (size_t i = 0; i < CONCURRENT_FRAMES; ++i) {
        mImageAvailableSemaphores[i] = mDevice.createSemaphoreUnique(vk::SemaphoreCreateInfo{}, nullptr,
                                                                     mDispatchLoader);
        mRenderFinishedSemaphores[i] = mDevice.createSemaphoreUnique(vk::SemaphoreCreateInfo{}, nullptr,
                                                                     mDispatchLoader);
        mSyncHostWithDeviceFence[i] = mDevice.createFenceUnique(
                vk::FenceCreateInfo{}.setFlags(vk::FenceCreateFlagBits::eSignaled), nullptr, mDispatchLoader);
    }
    ...
    mCommandPool = mDevice.createCommandPoolUnique(vk::CommandPoolCreateInfo{vk::CommandPoolCreateFlagBits::eTransient},
                                                   nullptr, mDispatchLoader);
...
double vklWaitForNextSwapchainImage() {
    ...
    vk::Result returnCode = mDevice.waitForFences(1u, &mSyncHostWithDeviceFence[mFrameInFlightIndex].get(), VK_TRUE,
                                                  std::numeric_limits<uint64_t>::max()); // Wait up to forever
    ...
    mCurrentSwapChainImageIndex = mDevice.acquireNextImageKHR(vk::SwapchainKHR{mSwapchainConfig.swapchainHandle},
                                                              std::numeric_limits<uint64_t>::max(),
                                                              mImageAvailableSemaphores[mFrameInFlightIndex].get(),
                                                              nullptr).value;
    ...
    if (mImagesInFlightFenceIndices[mCurrentSwapChainImageIndex] >= 0) {
        ...
        returnCode = mDevice.waitForFences(1u,
                                           &mSyncHostWithDeviceFence[mImagesInFlightFenceIndices[mCurrentSwapChainImageIndex]].get(),
                                           VK_TRUE, std::numeric_limits<uint64_t>::max()); // Wait up to forever
        VKL_CHECK_VULKAN_ERROR(static_cast<VkResult>(returnCode));
    ...
    mQueue.submit({vk::SubmitInfo{}
                           ...
                           .setPWaitDstStageMask(
                                   &mDstStages0) // It's the same destination stages that must wait on the image to become available.
...
void vklStartRecordingCommands() {
    ...
    auto numToErase = static_cast<size_t>(std::max(int{0}, static_cast<int>(mSingleUseCommandBuffers.size()) -
                                                           std::max(static_cast<int>(mSwapchainImageViews.size()),
                                                                    int{CONCURRENT_FRAMES})));
    assert(numToErase <= mSingleUseCommandBuffers.size());
    assert(numToErase < mSingleUseCommandBuffers.size() ||
           mFrameId < 10); // <-- Must be strictly smaller in later frames
    mSingleUseCommandBuffers.erase(std::begin(mSingleUseCommandBuffers),
                                   std::begin(mSingleUseCommandBuffers) + numToErase);
    ...
    cb.beginRenderPass(vk::RenderPassBeginInfo{
            ...
            static_cast<uint32_t>(mClearValues[mCurrentSwapChainImageIndex].size()),
            mClearValues[mCurrentSwapChainImageIndex].data()
...
void vklEndRecordingCommands() {
    ...
    if (mSingleUseCommandBuffers.empty()) {
        VKL_EXIT_WITH_ERROR(
                "There are no command buffers which could be recording.Have you called vklStartRecordingCommands beforehand?");
    }
...
VkFramebuffer vklGetFramebuffer(uint32_t i) {
    if (i >= mFramebuffers.size()) {
        VKL_EXIT_WITH_ERROR(
                "The given index[" + std::to_string(i) + "] is larger than the number of available framebuffers[" +
                std::to_string(mFramebuffers.size()) + "]");
    }
...
VkQueue vklGetQueue() {
    return static_cast<VkQueue>(mQueue);
}

VkImage vklCreateDeviceLocalImageWithBackingMemory(VkPhysicalDevice physical_device, VkDevice device, uint32_t width,
                                                   uint32_t height, VkFormat format, VkImageUsageFlags usage_flags,
    ...
    auto memoryAllocInfo = vk::MemoryAllocateInfo{}
            ...
            .setMemoryTypeIndex([&]() {
                ...
                if (-1 == selectedMemIndex) {
                    VKL_EXIT_WITH_ERROR(std::string("ERROR: Couldn't find suitable memory for image, requirements[") +
                                        std::to_string(memoryRequirements.alignment) + ", " +
                                        std::to_string(memoryRequirements.memoryTypeBits) + ", " +
                                        std::to_string(memoryRequirements.size) + "]");
                }
...
VkImage vklCreateDeviceLocalImageWithBackingMemory(VkPhysicalDevice physical_device, VkDevice device, uint32_t width,
                                                   uint32_t height, VkFormat format, VkImageUsageFlags usage_flags) {
    return vklCreateDeviceLocalImageWithBackingMemory(physical_device, device, width, height, format,
                                                      usage_flags, /* one layer: */ 1u, /* no flags: */{});
}
...
VkImage vklCreateDeviceLocalImageWithBackingMemory(uint32_t width, uint32_t height, VkFormat format,
    ...
    return vklCreateDeviceLocalImageWithBackingMemory(static_cast<VkPhysicalDevice>(mPhysicalDevice),
                                                      static_cast<VkDevice>(mDevice), width, height, format,
                                                      usage_flags);
}

VkImage vklCreateDeviceLocalImageWithBackingMemory(uint32_t width, uint32_t height, VkFormat format,
                                                   VkImageUsageFlags usage_flags, uint32_t array_layers,
    ...
    return vklCreateDeviceLocalImageWithBackingMemory(static_cast<VkPhysicalDevice>(mPhysicalDevice),
                                                      static_cast<VkDevice>(mDevice), width, height, format,
                                                      usage_flags, array_layers, flags);
}
...
void vklDestroyDeviceLocalImageAndItsBackingMemory(VkImage image) {
    ...
    if (mImagesWithBackingMemory.end() != search) {
        ...
        VKL_WARNING(
                "VkDeviceMemory for the given VkImage not found. Are you sure that you have created this buffer with vklCreateDeviceLocalImageWithBackingMemory(...)? Are you sure that you haven't already destroyed this VkImage?");
    }
...
VKAPI_ATTR VkBool32 VKAPI_CALL DebugUtilsMessengerCallback(
    ...
    if ((message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) != 0) {
        std::cout << "\nERROR:   messageIdNumber[" << callback_data->messageIdNumber << "], messageIdName["
                  << callback_data->pMessageIdName << "], message[" << callback_data->pMessage << "]" << std::endl;
    } else if ((message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) != 0) {
        std::cout << "\nWARNING: messageIdNumber[" << callback_data->messageIdNumber << "], messageIdName["
                  << callback_data->pMessageIdName << "], message[" << callback_data->pMessage << "]" << std::endl;
    } else if ((message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) != 0) {
        std::cout << "\nINFO:    messageIdNumber[" << callback_data->messageIdNumber << "], messageIdName["
                  << callback_data->pMessageIdName << "], message[" << callback_data->pMessage << "]" << std::endl;
    } else if ((message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) != 0) {
        std::cout << "\nVERBOSE: messageIdNumber[" << callback_data->messageIdNumber << "], messageIdName["
                  << callback_data->pMessageIdName << "], message[" << callback_data->pMessage << "]" << std::endl;
    }
    return VK_FALSE;
}
...
std::tuple<VklImageInfo, gli::texture2d> loadDdsImageWithGli(const char *file, uint32_t level) {
    ...
    if (level > gliTex.max_level()) {
        VKL_EXIT_WITH_ERROR(std::string("The specificed level[") + std::to_string(level) +
                            "] is not available in the DDS image file at path[" + file +
                            "]. Your are probably trying to load a level > 0 from a file which does not contain mipmap levels.");
    }
    ...
    switch (gliFormat) {
        ...
        default:
            VKL_EXIT_WITH_ERROR(
                    std::string("Unable to load DDS image file [") + file + "] due to an unsupported format.");
    }
...
VkBuffer vklLoadDdsImageFaceLevelIntoHostCoherentBuffer(const char *file, uint32_t face, uint32_t level) {
    ...
    auto host_coherent_buffer = vklCreateHostCoherentBufferWithBackingMemory(bufsize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
...
VklGeometryData vklLoadModelGeometry(const std::string &path_to_obj) {
    ...
    if (!tinyobj::LoadObj(&attributes, &shapes, &materials, &warning, &error, &sourceStream)) {
        VKL_EXIT_WITH_ERROR(
                "Failed attempt to load model in OBJ format from [" << path_to_obj << "]. Warning[" << warning
                                                                    << "], Error[" << error << "]");
    }
